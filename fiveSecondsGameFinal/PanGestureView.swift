...
open class PanGestureView: UIView {
    ...
    func addConstraintsToActionView(_ actionView:PanGestureActionView, direction:PanGestureViewSwipeDirection) {
        ...
        let constraints1 = NSLayoutConstraint.constraints(withVisualFormat: constraint1, options: [], metrics: [:], views: views as Any as! [String : Any])
        
        let constraint2 = "\(orientation2):|-0-[view]-0-|"
        let constraints2 = NSLayoutConstraint.constraints(withVisualFormat: constraint2, options: [], metrics: [:], views: views as Any as! [String : Any])
...
extension PanGestureView : UIGestureRecognizerDelegate {
        ...
        switch gesture.state {
            ...
            case .ended:
                ...
                if let actionView = self.actionViews[self.swipeDirection], let action = self.actions[self.swipeDirection], actionView.shouldTrigger  {
                    ...
                    UIView.animate(withDuration: 0.2, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 1, options: [UIViewAnimationOptions.curveEaseOut, UIViewAnimationOptions.allowUserInteraction], animations: { () -> Void in
    ...
    fileprivate func updatePosition(_ translation:CGPoint){
        ...
        if horizontalSwipeDirections.contains(swipeDirection){
            let elasticTranslation = elasticPoint(Float(translation.x), li: 400, lf: 1500)
            contentView.center.x = contentView.frame.size.width/2 + CGFloat(elasticTranslation)
        }
        else {
            let elasticTranslation = elasticPoint(Float(translation.y), li: 400, lf: 1500)
...
class PanGestureActionView: UIView {
    ...
    fileprivate func setupConstraints(){
        ...
        let hConstraints = NSLayoutConstraint.constraints(withVisualFormat: hConstraintString, options: [], metrics: [:], views: views as Any as! [String : Any])
        self.addConstraints(hConstraints)
        
        let vConstraintString = "\(orientation2):[imageView(44)]"
        let vConstraints = NSLayoutConstraint.constraints(withVisualFormat: vConstraintString, options: [], metrics: [:], views: views as Any as! [String : Any])
